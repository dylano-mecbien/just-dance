...
function createBracket(list: Participant[]): Bracket {
  const ordered = [...list].sort((a, b) => a.seed - b.seed).slice(0, 16).map((p) => p.id);
  const r16 = Array.from({ length: 8 }, (_, i) => ({ id: `r16-${i}`, p1: ordered[i * 2] || null, p2: ordered[i * 2 + 1] || null, winner: null }));
  return { r16, qf: Array.from({ length: 4 }, (_, i) => emptyMatch(`qf-${i}`)), sf: Array.from({ length: 2 }, (_, i) => emptyMatch(`sf-${i}`)), final: emptyMatch("final-0") };
} 
function fillOpenSlots(bracket: Bracket, list: Participant[]): Bracket {
  const next = structuredClone(bracket) as Bracket;
  const placed = new Set([...next.r16.flatMap((m) => [m.p1, m.p2])].filter(Boolean));
  const waiting = [...list].sort((a, b) => a.seed - b.seed).map((p) => p.id).filter((id) => !placed.has(id));
  next.r16.forEach((m) => { if (!m.p1 && waiting.length) m.p1 = waiting.shift()!; if (!m.p2 && waiting.length) m.p2 = waiting.shift()!; });
  return next;
}
...
function useContestData() {
  const [participants, setParticipants] = useState<Participant[]>(() => {
    try { return JSON.parse(localStorage.getItem("jd-participants") || "null")} catch { return seedParticipants; }
  });
  const [showRanking, setShowRanking] = useState(() => localStorage.getItem("jd-show-ranking") === "true");
  const [bracket, setBracket] = useState<Bracket>(() => { try { return JSON.parse(localStorage.getItem("jd-bracket") || "null") || createBracket(participants); } catch { return createBracket(participants); } });

  const refresh = async () => {
    try {
      const rows = await supabaseRequest("participants?select=*&order=seed.asc");
      if (rows) setParticipants(rows);
      const stateRows = await supabaseRequest("contest_state?select=*&id=eq.1");
      if (stateRows?.[0]) { if (stateRows[0].bracket) setBracket(stateRows[0].bracket); setShowRanking(Boolean(stateRows[0].show_ranking)); }
      if (rows || stateRows) return;
  ...
  useEffect(() => { setBracket((prev) => fillOpenSlots(prev, participants)); }, [participants]);
  useEffect(() => { localStorage.setItem("jd-bracket", JSON.stringify(bracket)); if (SUPABASE_URL && SUPABASE_KEY) supabaseRequest("contest_state", { method: "POST", headers: { Prefer: "resolution=merge-duplicates,return=representation" }, body: JSON.stringify({ id: 1, show_ranking: showRanking, bracket }) }).catch(() => undefined); }, [bracket, showRanking]);
  ...
  const chooseWinner = async (round: "r16" | "qf" | "sf" | "final", index: number, id: string) => {
    ...
    setBracket((current) => {
      const next = structuredClone(current) as Bracket;
      const matches = round === "final" ? [next.final] : next[round];
      const match = matches[index];
      if (!match || (match.p1 !== id && match.p2 !== id)) return current;
      match.winner = id;
      const advance = (target: Match, slot: "p1" | "p2") => { target[slot] = id; };
      if (round === "r16") advance(next.qf[Math.floor(index / 2)], index % 2 === 0 ? "p1" : "p2");
      if (round === "qf") advance(next.sf[Math.floor(index / 2)], index % 2 === 0 ? "p1" : "p2");
      if (round === "sf") advance(next.final, index === 0 ? "p1" : "p2");
      return next;
    });
    await updateStatus(id, round === "final" ? "winner" : "active");
    if (loserId) await updateStatus(loserId, "eliminated");
  };
  return { participants, showRanking, setShowRanking, addParticipant, updateStatus, bracket, chooseWinner };
}

function TournamentBracket({ bracket, participants, onWinner, adminMode = false }: { bracket: Bracket; participants: Participant[]; onWinner?: (round: "r16" | "qf" | "sf" | "final", index: number, id: string) => void; adminMode?: boolean }) {
  const byId = (id: string | null) => id ? participants.find((p) => p.id === id) : undefined;
  const player = (id: string | null, match: Match) => {
    const p = byId(id);
    const isWinner = Boolean(id && match.winner === id);
    const content = <><span className="bracket-player-name">{p?.pseudo || "À déterminer"}</span>{isWinner && <Check size={13} />}</>;
    if (adminMode && p && !match.winner) return <button className="bracket-player selectable" onClick={() => onWinner?.("r16", 0, p.id)}>{content}</button>;
    return <div className={isWinner ? "bracket-player won" : "bracket-player"}>{content}</div>;
  };
  const matchCard = (match: Match, round: "r16" | "qf" | "sf" | "final", index: number) => {
    const choose = (id: string | null) => id && onWinner?.(round, index, id);
    const item = (id: string | null) => { const p = byId(id); const isWinner = Boolean(id && match.winner === id); return adminMode && p && !match.winner ? <button className={isWinner ? "bracket-player selectable won" : "bracket-player selectable"} onClick={() => choose(id)}><span className="bracket-player-name">{p.pseudo}</span>{isWinner && <Check size={13} />}</button> : <div className={isWinner ? "bracket-player won" : "bracket-player"}><span className="bracket-player-name">{p?.pseudo || "À déterminer"}</span>{isWinner && <Check size={13} />}</div>; };
    return <div className={match.winner ? "bracket-match decided" : "bracket-match"} key={match.id}><div className="match-code">MATCH {String(index + 1).padStart(2, "0")}</div>{item(match.p1)}{item(match.p2)}</div>;
  };
  const round = (label: string, matches: Match[], name: "r16" | "qf" | "sf") => <div className={`bracket-round ${name}`}><div className="round-title">{label}</div><div className="round-matches">{matches.map((m, i) => matchCard(m, name, i))}</div></div>;
  return <div className="tournament-wrap"><div className="bracket-note"><Trophy size={17} /> TABLEAU À ÉLIMINATION DIRECTE <span>La gagnante avance d’une case</span></div><div className="tournament-scroll"><div className="tournament-bracket">{round("8ES DE FINALE", bracket.r16, "r16")}{round("QUARTS", bracket.qf, "qf")}{round("DEMI-FINALES", bracket.sf, "sf")}<div className="bracket-round final"><div className="round-title">FINALE</div><div className="round-matches">{matchCard(bracket.final, "final", 0)}<div className="trophy-slot"><img src={CROWN} alt="" /><span>GRANDE<br />CHAMPIONNE</span></div></div></div></div></div>{adminMode && <p className="bracket-admin-hint"><Zap size={14} /> Clique sur le nom de la gagnante de chaque match pour la faire avancer.</p>}</div>;
}
...
export default function Home() {
  ...
  const { participants, showRanking, setShowRanking, addParticipant, updateStatus, bracket, chooseWinner } = useContestData();
  ...
  const handleRegister = async (event: FormEvent<HTMLFormElement>) => {
    ...
    let photo_url = "";
    if (photo && photo.size) {
      if (photo.size > 5 * 1024 * 1024) { toast.error("Photo trop lourde", { description: "La photo doit faire moins de 5 Mo." }); return; }
      try { photo_url = await uploadParticipantPhoto(photo); } catch { toast.error("Photo non envoyée", { description: "Vérifiez le bucket participant-photos dans Supabase." }); return; }
    }
  ...
  return <div className="site-shell">
    <header className="topbar"><button className="brand" onClick={() => go("home")}><img src={MARK} alt="" /><span>OTAKU<span>ALL STAR</span></span></button><nav className={mobileOpen ? "nav-links open" : "nav-links"}><button onClick={() => go("home")}>Le programme</button><button onClick={() => document.getElementById("ranking")?.scrollIntoView({ behavior: "smooth" })}>Classement</button><button onClick={() => go("admin")} className="admin-link"><Shield size={15} /> Admin</button></nav><button className="menu-btn" onClick={() => setMobileOpen(!mobileOpen)} aria-label="Menu"><Menu size={22} /></button></header>

    {page === "home" && <>
      <main>
        <section className="hero-section"><div className="hero-art" style={{ backgroundImage: `linear-gradient(90deg, rgba(8,14,44,.98) 0%, rgba(8,14,44,.82) 42%, rgba(8,14,44,.12) 100%), url(${HERO})` }} /><div className="hero-copy"><div className="eyebrow"><span className="live-dot" /> Samedi 05 septembre · entrée libre</div><div className={SUPABASE_CONNECTED ? "connection-badge connected" : "connection-badge"}><span /> {SUPABASE_CONNECTED ? "Données synchronisées" : "Mode démo local"}</div><h1>La piste<br /><em>est à vous.</em></h1><p>Un concours Just Dance, des films, du challenge et un trophée pour la danseuse qui ira jusqu’au bout.</p><button className="primary-btn" onClick={() => go("register")}>Je m’inscris <ArrowRight size={18} /></button></div><div className="hero-stamp"><span>SAISON</span><strong>01</strong><small>Douala · 2026</small></div><div className="hero-bottom-note"><Zap size={16} /> Les participantes au Just Dance doivent impérativement être présentes à 12h20.</div></section>
